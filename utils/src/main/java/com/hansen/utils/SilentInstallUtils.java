...
public final class SilentInstallUtils {
    ...
    private static final int CODE_DM_NOT_FIND = -1;
    private static final int CODE_DM_NOT_SUPPORT = 1001;
        ...
        if (appInfo != null) {
            ...
            try {
                ...
                if (appPackageInfo != null && apkPackageInfo != null &&
                        !StringUtils.equalsIgnoreCase(appPackageInfo.sharedUserId, apkPackageInfo.sharedUserId)) {
...
//
    ...
    private static final class ResultCountDownTimer {

        private final CountDownLatch mCountDownLatch;
        private final AppUtils.AppInfo mApkInfo;
        private final HandlerThread mThread;
        private CountDownTimer mCountDownTimer;
        private boolean mResult;

        private ResultCountDownTimer(AppUtils.AppInfo apkInfo) {
            mCountDownLatch = new CountDownLatch(1);
            mApkInfo = apkInfo;
            mThread = new HandlerThread("CountDownTimer") {
                @Override
                protected void onLooperPrepared() {
                    super.onLooperPrepared();
                    mCountDownTimer = new CountDownTimer(TimeUnit.MINUTES.toMillis(3L), TimeUnit.SECONDS.toMillis(5L)) {
                        @Override
        ...
        private boolean isInstalled() {
            AppUtils.AppInfo appInfo = AppUtils.getAppInfo(mApkInfo.getPackageName());
            return appInfo != null && appInfo.getVersionCode() >= mApkInfo.getVersionCode();
        }
    ...
    private static final class InstallReceiver extends BroadcastReceiver {
        ...
        private IntentSender getIntentSender() {
            return PendingIntent.getBroadcast(mContext, ACTION.hashCode(),
                    new Intent(ACTION).putExtra("param", "test_msg"), PendingIntent.FLAG_UPDATE_CURRENT)
                    .getIntentSender();
        }
